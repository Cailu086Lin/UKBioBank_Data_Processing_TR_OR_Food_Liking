...
df<-read.csv("/media/cailu/Expansion1/ukb/bfilemaf01/variants_generegionmaf0.1/geno_pheno20240605.csv")%>%
  filter(!(eid %in% w57780_20241217$V1))

ass<-read.csv("/media/cailu/Expansion1/ukb/bfilemaf01/variants_generegionmaf0.1/foodliking_20241221.csv")%>%
  filter(FDR<=0.05)%>%
  mutate(marker = recode(marker, "rs10743938...889"="rs10743938...888", "rs2074464...236"= "rs2074464...235","rs7130086...516" ="rs7130086...515", "rs2173236...1036"="rs2173236...1035"))
...
result<-data.frame(trait=character(0), r2=numeric(0), p=character(0), N=numeric(0),SampleSize=numeric(0))
for (i in trait){
  temp<-data.frame(trait=NA, r2=NA, p=NA, N=NA, SampleSize=NA)
  list_snps<-ass[ass$trait==i,]$marker
  ...
  #temp$p <- 1 - pchisq(q=m2ll,df = dfn) # calculate p-value from -2*loglikelihood
  ...
  temp$SampleSize<-nrow(dat)
  result<-bind_rows(result, temp)
}


##clumping snp r2<0.2
result.2<-data.frame(trait=character(0), r2.2=numeric(0), p.2=character(0), N.2=numeric(0),SampleSize.2=numeric(0))
for (i in trait){
  temp<-data.frame(trait=NA, r2.2=NA, p.2=NA, N.2=NA,SampleSize.2=NA)
  subass<-ass[ass$trait==i,]
  list_snps<-subass[order(-subass$p.value),]$marker
  ...
  m2ll <- as.numeric((-2)*(logLik(m0)-logLik(m1)))
  dfn<-length(coef(m1))-length(coef(m0))
  #temp$p.2 <- 1 - pchisq(q=m2ll,df = dfn) # calculate p-value from -2*loglikelihood
  log_p_value<- pchisq(q=m2ll,df = dfn, lower.tail = F,log.p = TRUE) # calculate p-value from -2*loglikelihood
  log_p_value_mpfr <- mpfr(log_p_value, precBits = 256)
  temp$p.2<-formatMpfr(exp(log_p_value_mpfr),  digits = 3, scientific = TRUE)
  temp$N.2<-length(list_snps_clumped)
  temp$SampleSize.2<-nrow(dat)
  result.2<-bind_rows(result.2, temp)
}

##clumping snp r2<0.5
result.5<-data.frame(trait=character(0), r2.5=numeric(0), p.5=character(0), N.5=numeric(0),SampleSize.5=numeric(0))
for (i in trait){
  temp<-data.frame(trait=NA, r2.5=NA, p.5=NA, N.5=NA,SampleSize.5=NA)
  subass<-ass[ass$trait==i,]
  list_snps<-subass[order(-subass$p.value),]$marker
  ...
  m2ll <- as.numeric((-2)*(logLik(m0)-logLik(m1)))
  dfn<-length(coef(m1))-length(coef(m0))
  #temp$p.5 <- 1 - pchisq(q=m2ll,df = dfn) # calculate p-value from -2*loglikelihood
  log_p_value<- pchisq(q=m2ll,df = dfn, lower.tail = F,log.p = TRUE) # calculate p-value from -2*loglikelihood
  log_p_value_mpfr <- mpfr(log_p_value, precBits = 256)
  temp$p.5<-formatMpfr(exp(log_p_value_mpfr),  digits = 3, scientific = TRUE)
  temp$N.5<-length(list_snps_clumped)
  temp$SampleSize.5<-nrow(dat)
  result.5<-bind_rows(result.5, temp)
}

##clumping snp r2<0.8
result.8<-data.frame(trait=character(0), r2.8=numeric(0), p.8=character(0), N.8=numeric(0),SampleSize.8=numeric(0))
for (i in trait){
  temp<-data.frame(trait=NA, r2.8=NA, p.8=NA, N.8=NA,SampleSize.8=NA)
  subass<-ass[ass$trait==i,]
  list_snps<-subass[order(-subass$p.value),]$marker
...
write.csv(results_final, "/media/cailu/Expansion1/ukb/bfilemaf01/variants_generegionmaf0.1/variants_generegionmaf0.1/Food_liking_SNP_h2_202401223_updated.csv", row.names=F)
